        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-xl shadow-slate-200/40 ring-1 ring-slate-200 overflow-hidden">
                ...
                <div class="bg-slate-50/50 border-b border-slate-100 px-8 py-6">
                    ...
                    <p class="text-sm text-slate-500 mt-1">Fill out the information below to keep your project on track.
                    </p>
                </div>
                    ...
                    <form action="{{ isset($task) ? route('tasks.update', $task->id) : route('tasks.store') }}"
                        ...
                        <div class="group">
                            ...
                            <input type="text" name="title"
                                ...
                                placeholder="e.g., Fix database indexing"
                        ...
                        <div class="group">
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Upload Images (JPG,
                                PNG)</label>
                            <input type="file" name="images[]" multiple accept=".jpg,.jpeg,.png"
                                class="block w-full text-sm text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 ring-1 ring-inset {{ $errors->has('images.*') ? 'ring-rose-500' : 'ring-slate-300' }} rounded-lg p-1">
                            <p class="mt-1 text-xs text-slate-400">You can select multiple files at once.</p>
                            @error('images.*')
                                <p class="mt-1 text-sm text-rose-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="pt-6 mt-6 border-t border-slate-100 flex justify-end gap-3">
                            <a href="{{ route('tasks.index') }}"
                                class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md font-medium text-sm">Cancel</a>
                            <button type="submit"
                                class="px-4 py-2 bg-indigo-600 text-white rounded-md font-medium text-sm">
